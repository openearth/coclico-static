<template>
  ...
  >
    <v-list dense class="pa-0" dark>
      <v-list-item class="px-2" data-v-step="1">
        <!-- above @click="$router.push({ name: 'home' })" -->
        <!-- TODO: add routing -->
        <!-- <custom-icon name="coclico-full" class="icon--large" /> -->
        <v-list-item-content style="color: black">
          <v-list-item-title> CoCliCo </v-list-item-title>
        </v-list-item-content>
      </v-list-item>
      <v-divider />
      <v-list-item-group>
        <!-- above color="secondary" active-class="active-theme" -->
        <v-list-item class="list-elements" style="color: black">
          <!-- above v-for="item in getThemes"
          :key="item"
          @click="
            toggleTheme(item);
            openLayersCard();
          ...
          <div class="list-item">
            <v-list-item-icon>
              <custom-icon class="ml-8" />
              <!-- above :name="item" icon-folder="themes"  -->
            </v-list-item-icon>
            <v-list-item-title>
              <!-- above {{ item }} -->
            </v-list-item-title>
    ...
    <v-row class="mt-10" :align="align" no-gutters>
      <v-col>
        <div class="list-item">
      ...
      <v-col>
        <div class="list-item">
          <v-btn icon color="black">
            <!-- above @click="expandMenu = !expandMenu" -->
            <v-icon>mdi-dots-horizontal</v-icon>
          </v-btn>
          <v-subtitle>
            <!-- above v-show="expandMenu" class="text-caption" -->
            Less
          </v-subtitle>
          <v-subtitle>
            <!-- above v-show="!expandMenu" class="text-caption" -->
    ...
    <!-- <template #append>
      <div v-show="expandMenu">
        <v-list dense class="pa-0" color="terciary">
          ...
          <v-list-item @click="openCatalogPage">
            <div class="extra-list-item">
              ...
              <v-list-item-subtitle class="extra-list-item-text"
                >DATA CATALOG</v-list-item-subtitle
              >
            </div>
            <v-list-item-content>
          ...
          <v-list-item @click="openWorkbenchPage">
            <div class="extra-list-item-container">
              ...
              <v-list-item-subtitle class="extra-list-item-text"
                >WORKBENCH</v-list-item-subtitle
              >
            </div>
            <v-list-item-content>
